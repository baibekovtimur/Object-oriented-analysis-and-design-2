...
from .iterator_pattern import NearestUsersIterator
from .models import INTEREST_CATEGORIES, User, UserPoint
from .point_space import UserPointSpace


class SocialLabApp(tk.Tk):
    def __init__(self, space: UserPointSpace) -> None:
        super().__init__()
        self.space = space
        self.current_user_id = min(self.space.users.keys()) if self.space.users else None
        self.iterator_demo: NearestUsersIterator | None = None

        self.title("Iterator Pattern Lab")
        ...
        self.iterator_status_var = tk.StringVar(value="Iterator is not initialized yet.")
    ...
    def _build_layout(self) -> None:
        ...
        iterator_card = tk.Frame(table_card, bg="#1b2738")
        iterator_card.pack(fill="x", padx=12, pady=(0, 12))

        iterator_label = tk.Label(
            iterator_card,
            text="Iterator demo (manual next, no yield)",
            bg="#1b2738",
            fg=self._fg_main,
            font=("Segoe UI", 10, "bold"),
        )
        iterator_label.pack(anchor="w", padx=10, pady=(8, 4))

        iterator_status = tk.Label(
            iterator_card,
            textvariable=self.iterator_status_var,
            bg="#1b2738",
            fg=self._fg_muted,
            justify="left",
            wraplength=390,
        ...
        iterator_status.pack(anchor="w", padx=10, pady=(0, 6))

        iterator_actions = tk.Frame(iterator_card, bg="#1b2738")
        iterator_actions.pack(anchor="w", padx=10, pady=(0, 8))

        next_button = ttk.Button(iterator_actions, text="Iterator.next()", command=self._iterator_next)
        next_button.pack(side="left")

        reset_iterator_button = ttk.Button(
            iterator_actions,
            text="Iterator.reset()",
            command=self._iterator_reset,
        )
        reset_iterator_button.pack(side="left", padx=(8, 0))
    ...
    def _prepare_iterator_demo(self) -> None:
        if self.current_user_id is None:
            self.iterator_demo = None
            self.iterator_status_var.set("No base user selected.")
            return

        self.iterator_demo = self.space.create_iterator(self.current_user_id, limit=10)
        total = self.iterator_demo.total()
        if total == 0:
            self.iterator_status_var.set("Iterator created, but no candidates are available.")
        else:
            self.iterator_status_var.set(
                f"Iterator ready: position {self.iterator_demo.position()}/{total}."
            )

    def _iterator_next(self) -> None:
        if self.iterator_demo is None:
            self.iterator_status_var.set("Iterator is not initialized.")
            return

        if not self.iterator_demo.has_next():
            self.iterator_status_var.set(
                f"Iterator exhausted at {self.iterator_demo.position()}/{self.iterator_demo.total()}."
            )
            return

        recommendation = self.iterator_demo.next()
        self.iterator_status_var.set(
            f"Iterator.next() -> user_id={recommendation.user.user_id}, "
            f"distance={recommendation.distance:.3f}, "
            f"position={self.iterator_demo.position()}/{self.iterator_demo.total()}"
        )
    ...
    def _iterator_reset(self) -> None:
        ...
        self.iterator_status_var.set(
            f"Iterator reset: position {self.iterator_demo.position()}/{self.iterator_demo.total()}."
        )
